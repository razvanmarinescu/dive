...
def logPriorShiftInform(shift, paramsPriorShift):
  ...
  likAlpha = scipy.stats.gamma.pdf(alpha, a=shape, scale=1.0/rate)
  likBeta = scipy.stats.norm.pdf(beta, loc=mu, scale=std)

  if likAlpha == 0 or likBeta == 0:
    return -float('inf')

  return np.log(likAlpha) + np.log(likBeta)
...
def setPrior(params, informativePrior, mean_gamma_alpha=0.7,
  ...
  if informativePrior:
    ...
    # mean_gamma_alpha = 1
    # std_gamma_alpha = 0.4
    shape_alpha = (mean_gamma_alpha ** 2) / std_gamma_alpha
    rate_alpha = mean_gamma_alpha / std_gamma_alpha
    ...
    params['logPriorThetaFunc'] = logPriorThetaInform
    params['logPriorThetaFuncDeriv'] = logPriorThetaInformDeriv
