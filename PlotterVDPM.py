...
class PlotterVDPM:
  ...
  def plotTrajWeightedDataMean(self, data, diag, dps, longData, longDiag, longDPS, thetas, variances, clustProbBCColNorm, plotTrajParams,
    ...
    if orderClust:
      clustOrderInd = orderTrajBySlope(thetas)
    else:
  ...
  def plotTrajSamplesInOneNoData(self, data, diag, dps, thetas, variances, clustProbBCColNorm, plotTrajParams,
     trajFunc, replaceFigMode=True, thetasSamplesClust=None, showConfInt=True,
     colorTitle=True, yLimUseData=False,
     windowSize=(500,400), clustColsHues=None):
  ...
  def plotTrajWeightedDataMeanTrueParams(self, data, diag, dps, thetas, variances, clustProbBCColNorm,
    ...
    legend = pl.figlegend(h[-3:], axisLabels[-3:], loc='lower center', ncol=plotTrajParams['legendCols'], labelspacing=0. )
    # set the linewidth of each legend object
    for i,legobj in enumerate(legend.legendHandles):
      legobj.set_linewidth(4.0)
      #legobj.set_color(plotTrajParams['diagColors'][diagNrs[i]])
    ...
    if fontsize:
      ...
      # pl.tight_layout()
      pl.subplots_adjust(left = None, bottom = 0.3, right = None, top = None, wspace = 0.05, hspace = None)
  ...
  def plotSubShiftsTrue(self, subShifts, subShiftsTrue, dpsLong, dpsLongTrue, plotTrajParams, replaceFigMode=True,
    ...
    pl.xlabel("true DPS", fontsize=fontsize)
    pl.ylabel("estimated DPS", fontsize=fontsize)
    axes = pl.gca()
...
class PlotterVDPMSynth(PlotterVDPM):
  ...
  def plotSynthResOneExp(self, numbers, stds, xTickValues, xLabelStr, makeInts=False, adjLeft=0.15):
    """
    ...
    stds = np.sqrt((np.array(stds) ** 2)/1000) # compute the Std of the sample mean by classical formula

    pl.errorbar(range(len(numbers)), numbers, yerr=stds, linewidth=3)
    ax = pl.gca()
    ...
    pl.ylabel('Percentage of correct vertices', fontsize=fs)
